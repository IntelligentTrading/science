...
class ComparativeEvaluation:
    ...
    def __init__(self, buy_signals, sell_signals, num_buy, num_sell, signal_combination_mode,
        ...
        self.signal_combination_mode = signal_combination_mode
    ...
    def check_signal_combination_mode(self, buy_signal_set, sell_signal_set):
        if self.signal_combination_mode == SignalCombinationMode.ANY:
            return True
        buy_types = set([ALL_SIGNALS[x].signal for x in buy_signal_set])
        sell_types = set([ALL_SIGNALS[x].signal for x in sell_signal_set])
        if self.signal_combination_mode == SignalCombinationMode.SAME_TYPE:
            return buy_types == sell_types and len(buy_types) == 1
        elif self.signal_combination_mode == SignalCombinationMode.SAME_TYPE_AND_STRENGTH:
    ...
    def generate_evaluations(self, transaction_currency, counter_currency, horizon):
        ...
        for buy, sell in buy_sell_pairs:
            if not self.check_signal_combination_mode(buy, sell):
                continue
            signal_set = list(buy)
